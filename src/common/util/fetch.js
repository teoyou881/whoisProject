...
export function makeFetchSaga({ fetchSaga, canCache, getTotalCount = (res) => res?.totalCount }) {
    return function* (action) {
        ...
        const iterStack = [];
        let iter = fetchSaga(action, page); //만든 fetch 함수를 생성해서 제너레이터를 받는다.
        let res;
        let checkSlowTask;
        let params;
        while (true) {
            const { value, done } = iter.next(res);
            //사가 함수 안에서 또 다른 사가 함수를 호출하는 경우
            ...
            if (getIsCallEffect(value) && value.payload.fn === callApi) {
                ...
                let apiResult =
                    canCache && apiCache.has(cacheKey) ? apiCache.get(cacheKey) : undefined;
